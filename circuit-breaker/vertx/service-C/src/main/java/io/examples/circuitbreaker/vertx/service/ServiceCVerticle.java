...
/**
 * Verticle for service C
 *
 * @author Gary Cheng
 */
public class ServiceCVerticle extends AbstractVerticle {
    private static final Logger logger = LoggerFactory.getLogger(ServiceCVerticle.class);
    ...
    public void start(Future<Void> startFuture) {
        logger.debug("Starting Service C Verticle");
        int port = this.config().getInteger(KEY_PORT, 9082);
        ...
        server.rxListen(port)
                ...
                .doAfterSuccess(r -> logger.debug("ServiceC published"))
    ...
    public void stop(Future<Void> stopFuture) {
        if (null != this.publishedRecord) {
            discovery.rxUnpublish(this.publishedRecord.getRegistration())
                    .doOnComplete(() -> logger.debug("ServiceC unPublished"))
    ...
    private Router router(Vertx vertx) {
        ...
        router.route(SERVICE_C_PATH).handler(this::handleService);
        return router;
    }

    private void handleService(RoutingContext context) {
        JsonObject result = new JsonObject();
        result.put("ServiceC result", "Welcome to Service C");
    ...
    private Record createRecord(JsonObject config) {
        return HttpEndpoint.createRecord(config.getString(KEY_SERVICE), config.getString(KEY_HOST),
                config.getInteger(KEY_PORT), SERVICE_C_PATH);
    }
}
